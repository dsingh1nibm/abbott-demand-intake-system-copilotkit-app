...
async function submitIntake(form: FormData, userId?: number) {
  const res = await fetch("http://localhost:8000/intake-records", {
    method: "POST",
...
function MoMModal({ masterData, onClose, onSuccess, userId }: {
  ...
  const handleFile = async (f: File) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8000/parse-mom", { method: "POST", body: fd });
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/master-data")
      .then((r) => r.json())
      .then(setMasterData)
      .catch(() => { });
  }, []);
  ...
  const NEW_INTAKE_KEYWORDS = [
    ...
    "new demand", "create demand", "submit demand", "add demand",
  ];
    ...
    try {
      const res = await fetch("http://localhost:8000/chat", {
        ...
        body: JSON.stringify({
          messages: updated,
          user_id: user?.UserID,
          intent: isEditIntent ? "edit_intake" : "",
        }),
      });
      const data = await res.json();
              ...
              <div key={i}>
                ...
                {m.type === "intake-options" && i === messages.length - 1 && (
                  <div className="option-btns">
                    <button className="option-btn" onClick={() => setShowFormModal(true)}>📋 By Form</button>
                    <button className="option-btn" onClick={startManual}>✍️ Manually</button>
                    <button className="option-btn" onClick={() => setShowMomModal(true)}>📄 By MoM Document</button>
                  </div>
                )}
              </div>
            ))}
